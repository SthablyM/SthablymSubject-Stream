...
export const NSC_SUBJECTS = [
  { id: "english",        label: "English HL / FAL",             lo: false },
...
// ─────────────────────────────────────────────────────────────────────────────
export const markToAPS = (m) => {
  m = Number(m);
  return m >= 80 ? 7 : m >= 70 ? 6 : m >= 60 ? 5 : m >= 50 ? 4 : m >= 40 ? 3 : m >= 30 ? 2 : 1;
};

export const markToSymbol = (m) => {
  m = Number(m);
  return m >= 80 ? "A" : m >= 70 ? "B" : m >= 60 ? "C" : m >= 50 ? "D" : m >= 40 ? "E" : m >= 30 ? "F" : "G";
};
...
export const mathsLabel = (marks = {}) =>
  typeof marks.puremaths === "number"
    ? "Pure Mathematics"
    : typeof marks.techmaths === "number"
    ? "Technical Mathematics"
    : typeof marks.mathslit === "number"
...
const SYM_PTS = { A: 7, B: 6, C: 5, D: 4, E: 3, F: 2, G: 1 };

const apsBarColor = (a) => (a >= 30 ? "#16a34a" : a >= 22 ? "#d97706" : "#dc2626");
const apsCellColor = (a) => (a >= 6 ? "#16a34a" : a >= 4 ? "#d97706" : "#dc2626");

// ─────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
export default function APSCalculator({ onChange }) {
  // Committed numeric marks — only updated on blur/Enter
  const [marks, setMarks] = useState({});

  // Raw string values tracked per input while the user is typing.
  // Keeping these separate from `marks` prevents React from resetting
  // the input value (and cursor position) on every keystroke.
  const [rawInputs, setRawInputs] = useState({});

  // ── handlers ──────────────────────────────────────────────────────────────

  // While typing: only update the raw string buffer. Do NOT touch `marks`.
  const handleChange = useCallback((id, value) => {
    setRawInputs((prev) => ({ ...prev, [id]: value }));
  }, []);

  // On blur (or Enter): validate, clamp, then commit to marks.
    ...
    (id, rawValue) => {
      ...
      setMarks((prev) => {
        ...
        if (trimmed === "" || isNaN(Number(trimmed))) {
          // Empty or non-numeric — clear the mark
          delete next[id];
        } else {
          const clamped = Math.min(100, Math.max(0, Math.round(Number(trimmed))));
          next[id] = clamped;
          // Normalise the raw buffer to the clamped value
          setRawInputs((r) => ({ ...r, [id]: String(clamped) }));
        }
  ...
  const handleBlur = useCallback(
    (id) => {
      commitMark(id, rawInputs[id] ?? "");
    },
    [commitMark, rawInputs]
  );
  ...
  const statusLabel = !hasAny
    ...
    : aps >= 30
    ? "🟢 Strong — most programmes open"
    : aps >= 22
    ? "🟡 Good — many programmes available"
    ...
    <div style={s.wrap}>
      ...
      <div style={s.header}>
        ...
        <div style={s.scoreBlock}>
          <div style={{ ...s.scoreBig, color: hasAny ? barColor : "rgba(255,255,255,.5)" }}>
            {aps}
            <span style={s.scoreMax}>/42</span>
          </div>
          <div style={s.scoreLabel}>APS score</div>
        </div>
      </div>
      ...
      <div
        style={{
          ...s.statusBanner,
          borderColor: hasAny ? barColor : "#e2e8f0",
        }}
      >
        {statusLabel}
      </div>
      ...
      <div style={s.symRef}>
        {Object.entries(SYM_STYLES).map(([sym, st]) => (
          <span
            key={sym}
            style={{ ...s.symChip, background: st.bg, color: st.color }}
          >
            {sym} = {SYM_PTS[sym]}
          </span>
        ))}
        <span style={s.symNote}>
          Mark range: A(80%+) B(70) C(60) D(50) E(40) F(30) G(&lt;30)
        </span>
      </div>
      ...
      {NSC_SUBJECTS.map((sub, i) => {
        ...
        // Show the raw string while the user is typing; fall back to the
        // committed value (as string) or empty string when nothing is entered.
        const inputValue = rawInputs[sub.id] !== undefined
          ? rawInputs[sub.id]
          : has
          ? String(m)
          : "";
        ...
        return (
          ...
          >
            ...
            {/*
              KEY FIX:
              - type="text" inputMode="numeric" — prevents browser number-input
                quirks (spin buttons, scientific notation, cursor resets).
              - value={inputValue} — reflects raw string while typing.
              - onChange → only updates rawInputs (no mark commit, no re-render).
              - onBlur  → commits and triggers a re-render.
              - onKeyDown Enter → commits and blurs.
            */}
            <input
              type="text"
              inputMode="numeric"
              ...
              style={{
                ...s.markInp,
                borderColor: has ? apsCellColor(pts || 1) : "#e2e8f0",
              }}
            />
            ...
            <span
              style={{
                ...s.apsCell,
                color: pts ? apsCellColor(pts) : "#94a3b8",
              }}
            >
              {sub.lo ? (has ? "LO" : "—") : pts || "—"}
            </span>
      ...
      {hasAny && (
        <div style={s.summary}>
          ...
          <div style={s.summaryRow}>
            ...
            <span style={{ ...s.summaryVal, fontSize: 22, color: barColor }}>
              {aps} / 42
            </span>
          </div>
          <div style={s.summaryRow}>
...
const s = {
  ...
  tableHead: {
    display: "grid",
    gridTemplateColumns: "1fr 80px 60px 40px",
    gap: 8,
    padding: "8px 20px",
    background: "#f1f5f9",
