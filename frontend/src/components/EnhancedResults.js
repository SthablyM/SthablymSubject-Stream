...
export default function EnhancedResults({ streamScores, mathResults, student }) {
  ...
  // Grade 8: land directly on Past Papers tab
  // Grade 9: land on results but Past Papers tab is visible
  // Grade 10+: standard behaviour
  const defaultSection = student?.goToPastPapers || grade === 8 ? "pastpapers" : "results";
  ...
  // Grade 8: show Past Papers directly — no stream quiz results needed
  const isGrade8 = grade === 8;
  const isGrade9 = grade === 9;

  // For grade 8, streamScores may be empty — that's fine, we route them to Past Papers
  if (!isGrade8 && (!streamScores || Object.keys(streamScores).length === 0)) {
    return (
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Segoe UI',sans-serif", background:"#e0f2fe" }}>
  ...
  // Safe defaults for grade 8 (no stream scores)
  const sorted    = streamScores ? Object.entries(streamScores).sort((a,b) => b[1]-a[1]) : [];
  const topStream = sorted[0]?.[0] || "Science Stream";
  const topScore  = sorted[0]?.[1] || 0;
  const second    = sorted[1];
  const info      = SA_STREAMS[topStream] || SA_STREAMS["Science Stream"];
  ...
  const NAV = isGrade8
    ? [
        { id:"pastpapers", label:"📝 Practice Questions" },
  ...
  // ── Banner colour & label ─────────────────────────────────────────────────
  const bannerColor = isGrade8 ? "#0f766e" : isGrade9 ? "#7c3aed" : info.color;
  const bannerIcon  = isGrade8 ? "🌱" : isGrade9 ? "🌿" : info.icon;
  const bannerTitle = isGrade8
    ? "Grade 8 Practice Questions"
    : isGrade9
    ? `${student?.name ? student.name + "'s" : "Your"} Stream Match`
    : topStream;
  const bannerLabel = isGrade8
    ? `${student?.name || "Student"} · ${student?.school || ""}`
    : isGrade9
    ? `Grade 9 — Choose your Grade 10 subjects`
    : isHighSchool
    ? `Grade ${student?.grade} — Subject Stream`
    : `${student?.name ? student.name + "'s" : "Your"} Recommended Stream`;

  return (
    <div style={s.page}>
      {/* ── Banner ── */}
      <div style={{ ...s.banner, background: bannerColor }}>
        <div style={s.bannerInner}>
          <div style={s.bannerLeft}>
            <span style={s.bannerIcon}>{bannerIcon}</span>
            <div>
              <div style={{ marginBottom:4 }}><StablymLogo variant="dark" size="xs"/></div>
              <p style={s.bannerLabel}>{bannerLabel}</p>
              <h1 style={s.bannerStream}>{bannerTitle}</h1>
      ...
      <div style={s.nav}>
        {NAV.map(n => (
          <button key={n.id}
            style={{ ...s.navBtn, ...(activeSection === n.id ? { ...s.navActive, borderBottomColor: bannerColor, color: bannerColor } : {}) }}
      ...
      <div style={{ ...s.content, maxWidth: activeSection === "teacher" ? 1100 : 800 }}>
        ...
        {activeSection === "pastpapers" && isGrade8 && (
          <div style={{ background:"#f0fdfa", border:"1px solid #99f6e4", borderRadius:12, padding:"14px 16px", marginBottom:16, display:"flex", gap:12, alignItems:"flex-start" }}>
            ...
            <div>
              <b style={{ fontSize:14, color:"#0f766e" }}>Welcome, {student?.name || "learner"}! Let's practise your Grade 8 subjects.</b>
              <p style={{ margin:"4px 0 0", fontSize:13, color:"#0f766e", lineHeight:1.6 }}>
                Choose a subject below and start a <b>10-question timed quiz</b>. You'll get instant feedback and detailed explanations after every answer. Great preparation for your tests and exams!
              </p>
            </div>
          </div>
        )}

        {/* ══ GRADE 9 PAST PAPERS INFO ══ */}
        {activeSection === "pastpapers" && isGrade9 && (
          <div style={{ background:"#faf5ff", border:"1px solid #e9d5ff", borderRadius:12, padding:"14px 16px", marginBottom:16, display:"flex", gap:12, alignItems:"flex-start" }}>
            <span style={{ fontSize:24, flexShrink:0 }}>🌿</span>
            <div>
              <b style={{ fontSize:14, color:"#7c3aed" }}>Grade 9 Practice Questions</b>
              <p style={{ margin:"4px 0 0", fontSize:13, color:"#6b21a8", lineHeight:1.6 }}>
                Practise your Grade 9 subjects to prepare for end-of-year exams — and get a head start on the Grade 10 work you'll be doing next year!
              </p>
            </div>
          </div>
        )}
        ...
        {/* ══ RESULTS ══ */}
        {activeSection === "results" && !isGrade8 && (
          <div>
            {isHighSchool && (
              <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:12, padding:"12px 16px", marginBottom:16, display:"flex", gap:12, alignItems:"flex-start" }}>
                ...
                <div>
                  ...
                  <p style={{ margin:"4px 0 0", fontSize:13, color:"#1e40af", lineHeight:1.6 }}>Your stream was identified from your subject marks. The <b>"What Can I Study?"</b> tab shows university programmes you qualify for based on your APS of <b>{displayAPS}/42</b>.</p>
                </div>
              </div>
            )}
            {isGrade9 && (
              <div style={{ background:"#faf5ff", border:"1px solid #e9d5ff", borderRadius:12, padding:"12px 16px", marginBottom:16, display:"flex", gap:12, alignItems:"flex-start" }}>
                <span style={{ fontSize:22, flexShrink:0 }}>🌿</span>
                <div>
                  <b style={{ fontSize:14, color:"#7c3aed" }}>Your recommended stream for Grade 10</b>
                  <p style={{ margin:"4px 0 0", fontSize:13, color:"#6b21a8", lineHeight:1.6 }}>
                    Based on your quiz answers, <b>{topStream}</b> suits you best. Talk to your school counsellor and parents before making your final subject choice. Use the <b>Past Papers</b> tab to practice Grade 9 questions!
                  </p>
                </div>
              </div>
            )}
                  ...
                  {Object.entries(activeMarks).filter(([,v]) => v > 0).map(([id, mark]) => {
                    ...
                    return (
                      <div key={id} style={{ ...s.gradeChip, background:col }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color:tc }}>{markToSymbol(mark)} · APS {markToAPS(mark)}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        {/* ══ APS CALCULATOR ══ */}
        {activeSection === "aps" && isHighSchool && (
          <div>
        ...
        {/* ══ UNIVERSITIES ══ */}
        {activeSection === "universities" && isHighSchool && (
          <UniversityFinder stream={topStream} aps={displayAPS} marks={activeMarks}/>
        )}

        {/* ══ DEADLINES ══ */}
        {activeSection === "deadlines" && isHighSchool && (
          <UniversityFinder stream={topStream} aps={displayAPS} marks={activeMarks}/>
        )}

        {/* ══ BURSARIES ══ */}
        {activeSection === "bursaries" && isHighSchool && (
          <div>
            <div style={s.nsfasCard}>
              ...
              <div>
                ...
                <p style={{ margin:0, fontSize:13, color:"#713f12", lineHeight:1.6 }}>
                  NSFAS covers <b>full tuition, accommodation, and meals</b>. Apply at <a href="https://nsfas.org.za" target="_blank" rel="noopener noreferrer" style={{ color:"#92400e", fontWeight:700 }}>nsfas.org.za ↗</a> before <b>30 November</b> each year.
                </p>
              </div>
            </div>
            <UniversityFinder stream={topStream} aps={displayAPS} marks={activeMarks}/>
        ...
        {activeSection === "pastpapers" && (
          <PastPapersQuiz
            ...
            onBack={() => setActiveSection(isGrade8 ? "pastpapers" : "results")}
          />
        )}
          ...
          <div>
            {isGrade8 && (
              <div style={{ ...s.card, borderLeft:"4px solid #0f766e" }}>
                <h3 style={s.cardTitle}>🌱 Grade 8 Advice</h3>
                <p style={{ fontSize:13, color:"#374151", margin:0, lineHeight:1.7 }}>
                  Grade 8 is your foundation year. Work hard in <b>Mathematics, English, and Natural Sciences</b> — these subjects matter most when you choose your Grade 10 stream next year. Practice past questions regularly and ask your teacher when you're stuck.
                </p>
              </div>
            )}
            {isGrade9 && (
              <div style={{ ...s.card, borderLeft:"4px solid #7c3aed" }}>
                <h3 style={s.cardTitle}>🌿 Grade 9 — Preparing for Your Stream Choice</h3>
                <p style={{ fontSize:13, color:"#374151", margin:0, lineHeight:1.7 }}>
                  Your Grade 10 subject choice is one of the most important decisions you'll make. Based on your quiz, the <b>{topStream}</b> suits your interests best. Speak to your school counsellor, research careers in that stream, and make sure you understand what each subject involves before deciding.
                </p>
              </div>
            )}
            {displayAPS > 0 && !isGrade8 && !isGrade9 && (
              <div style={{ ...s.card, borderLeft:`4px solid ${displayAPS>=30?"#16a34a":displayAPS>=22?"#d97706":"#dc2626"}` }}>
                ...
                <p style={{ fontSize:13, color:"#374151", margin:0 }}>
                  {displayAPS>=30?"🟢 Strong APS! You qualify for most university programmes. Keep working hard."
                  :displayAPS>=22?"🟡 Good APS. Focus on improving your weakest subjects."
                  :"🔴 Your APS needs improvement. Speak to your teacher about a study plan."}
                </p>
              </div>
            )}
            {!isGrade8 && (
              <div style={s.card}>
                <h3 style={s.cardTitle}>🗺️ Your Roadmap {!isGrade9 && `for ${topStream}`}</h3>
                <div style={s.roadmap}>
                  {(isGrade9 ? getGrade9Roadmap(topStream) : getRoadmap(topStream)).map((step, i) => (
                    <div key={i} style={s.roadmapStep}>
                      <div style={{ ...s.roadmapNum, background: isGrade9 ? "#7c3aed" : info.color }}>{i+1}</div>
            ...
            <div style={s.card}>
              <h3 style={s.cardTitle}>📚 Study Tips</h3>
              {getStudyTips(isGrade8 ? null : topStream, student).map((tip, i) => (
            ...
            <div style={{ ...s.card, background:`linear-gradient(135deg,${isGrade8?"#f0fdfa":isGrade9?"#faf5ff":info.bg},#fff)`, border:`1px solid ${bannerColor}40` }}>
              ...
              <p style={{ textAlign:"center", color:bannerColor, fontWeight:700, marginTop:8, fontSize:13 }}>— Stablym</p>
            </div>
          </div>
        )}
...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
function getGrade9Roadmap(stream) {
  return [
    { title:"Right now (Grade 9)", desc:"Do well in your current subjects — especially Maths, English, and EMS. These form the base for every stream." },
    { title:"Choose your subjects", desc:`Your quiz suggests the ${stream}. Research what each subject involves and talk to your teacher before deciding.` },
    { title:"Grade 10 — first year of NSC", desc:"Your Grade 10 marks count toward your final NSC! Start strong, attend extra classes, and do practice tests." },
    { title:"Grade 11 & 12", desc:"Build your APS, apply for bursaries, and start researching university programmes that match your stream." },
    { title:"Beyond school", desc:"University, TVET College, or Learnership — there are many paths. Choose what suits your interests and strengths." },
  ];
}
...
function getStudyTips(stream, student) {
  const base = [
    ...
    { emoji:"🙋", text:"Ask your teacher for help as soon as you don't understand something." },
    { emoji:"📱", text:"Use free apps like Khan Academy, Maths Genie, and YouTube for extra explanations." },
    { emoji:"👥", text:"Form a study group — teaching others is one of the best ways to learn." },
    { emoji:"📝", text:"Use the Past Papers tab to practise exam-style questions with instant feedback." },
  ];
  const extra = {
    "Science Stream":                 [{ emoji:"🔬", text:"Do past papers for Maths and Science from Grade 10 — patterns repeat in the NSC." }],
    "Commerce Stream":                [{ emoji:"💹", text:"Follow business news — real-world context makes Accounting click." }],
    "Humanities Stream":              [{ emoji:"✍️", text:"Write practice essays regularly and ask your teacher to mark them." }],
    "Engineering / Technical Stream": [{ emoji:"🔧", text:"Build things! Even small hobby electronics or woodwork projects teach more than textbooks." }],
  };
...
// ─────────────────────────────────────────────────────────────────────────────
const s = {
  page:          { minHeight:"100vh", background:"#e0f2fe", fontFamily:"'Segoe UI',sans-serif", paddingBottom:40 },
  ...
  bannerStream:  { fontSize:26, fontWeight:900, color:"#fff", margin:0 },
  bannerAps:     { fontSize:13, color:"rgba(255,255,255,.8)", margin:"4px 0 0" },
  pdfBtn:        { padding:"12px 20px", background:"rgba(255,255,255,.15)", border:"2px solid rgba(255,255,255,.6)", color:"#fff", borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer" },
  ...
  apsTotal:      { display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 20px", background:"#f0f9ff", borderRadius:12, border:"1px solid #bfdbfe" },
